...
namespace Point
{
    ...
    public partial class MainWindow : Window
    {
        ...
        public abstract class MatematikselIfadeler
        {
            public abstract void bilgiGöster();
            public abstract void islemGöster();
            public abstract void tipGöster();

            public void islemYap() //template metodu burası
            {
                sayiKosulu();
                bilgiGöster();
                islemGöster();
            ...
            private void sayiKosulu()
            {
                MessageBox.Show("Sayılar reel Sayı olmalıdır..!"); //Ekstra ekleme yaptım desen icin
            }
        }
        public class forVector : MatematikselIfadeler
        {
            private string resultType, operationString, syntaxString;
            private string bilgi, tip, islem;
            public forVector()
            {
                syntaxString = "pointResult = point1 + vector1";
                resultType = "Point";
                operationString = "Adding a Point and Vector";
                //BU KISIMDA BU KODLARI CLASS A ERİŞİM SAGLAMAK İCİN YAZMAM GEREKTİ.
            }
            public override void bilgiGöster()
            {
                MessageBox.Show("Vector için bilgi göster komutu calısır");
                this.bilgi = operationString;
                MessageBox.Show("/n");
                MessageBox.Show("Vector bilgisi: ", this.bilgi);
            }

            public override void islemGöster()
            {
                MessageBox.Show("Vector için islem göster komutu calısır");
                this.islem = syntaxString;
                MessageBox.Show("İslem bilgisi: ", this.islem);
            }
            public override void tipGöster()
            {
                MessageBox.Show("Vector icin tip göster komutu calısır");
                this.tip = resultType;
                MessageBox.Show("İslem tipi: ", this.tip);
            }
        }

        public class forMatrix : MatematikselIfadeler
        {
            private string resultType, operationString, syntaxString;
            private string bilgi, tip, islem;
            public forMatrix()
            ...
            public override void bilgiGöster()
            {
                MessageBox.Show("Matris için bilgi göster komutu calısır");
                this.bilgi = operationString;
                MessageBox.Show("/n");
                MessageBox.Show("Matrix bilgisi: ", this.bilgi);
            }

            public override void islemGöster()
            {
                MessageBox.Show("Matrix için bilgi göster komutu calısır");
                this.islem = syntaxString;
                MessageBox.Show("/n");
                MessageBox.Show("İslem bilgisi: ", this.islem);
            }
                ...
                case "rb1":
                    {
                        ...
                        // Returns a Point.
                        //BURAYA İLK VECTOR İSLEMİNİ ,RADİOBOX YERİNE UYGULADIK
                        MatematikselIfadeler mat = new forVector();
                        mat.islemYap(); //islemyap() kısmı pattern kısmı
                        break;
                    }
                ...
                case "rb2":
                    {
                        ...
                        // Returns a Point.  
                        //BURAYA MATRİX İSLEMİNİ UYGULUYORUZ
                        MatematikselIfadeler mat = new forMatrix();
                        mat.islemYap(); //islemyap() template metodu oluyor yani pattern kısmı
                        break;
                    }
