...
namespace Matrix
{
    ...
    public partial class MainWindow : Window
    {
        ...
        // SADECE XML KODU İLE YAPILMIS BİZDE BU BUTONA(METODA) PROXY UYGULAMAK İSTEDİK

        public interface IMatrisIslem
        {
            void Topla();
            void Carp();
        }

        public class RealMatrix : MainWindow, IMatrisIslem
        {
            private string syntaxString, resultType, operationString;
            public void Topla()
            {
                MessageBox.Show("Toplama işlemi yapılıyor..");
                ...
                MessageBox.Show("toplama oldu");
            }
            public void Carp()
            {
                MessageBox.Show("Carpma işlemi yapılıyor..");
                var matrix1 = new System.Windows.Media.Matrix(5, 10, 15, 20, 25, 30);
                var matrix2 = new System.Windows.Media.Matrix(2, 4, 6, 8, 10, 12);
                ...
                MessageBox.Show("Carpma oldu");
            }
        }

        public class ProxyMatrix : IMatrisIslem
        {
            private RealMatrix matris;
            public void Carp()
            {
                if (matris == null)
                {
                    matris = new RealMatrix();
                }
                matris.Carp();
            }
            ...
            public void Topla()
            {
                if (matris == null)
                {
                    matris = new RealMatrix(); // Real yani ana class null olursa yeniden olusturuyor.
                }
                matris.Topla();
            }
        }
        public void PerformOperation(object sender, RoutedEventArgs e)
        {
            ...
            switch (li?.Name)
            {
                ...
                case "rb13":
                    {
                        // Bir Matrisi diger matrise ekleme kodları vardı burada..

                        // Appends a Matrix to another Matrix
                        ProxyMatrix proxy = new ProxyMatrix();
                        proxy.Topla();
                        break;
                    }
                case "rb14":
                ...
                case "rb19":
                    {
                        // Bir matris ile diger bir matrisi carpma kodları var idi burada..
